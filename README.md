...
8. [BLE adattovábbítás – transzparens híd](#8-ble-adattovábbítás--transzparens-híd)
9. [WiFi funkciók és webes felület](#9-wifi-funkciók-és-webes-felület)
10. [WebSerial parancsok](#10-webserial-parancsok)
11. [LED állapotjelzés](#11-led-állapotjelzés)
12. [Nyomógomb funkciók](#12-nyomógomb-funkciók)
13. [Watchdog és Deep Sleep](#13-watchdog-és-deep-sleep)
14. [EEPROM – Beállítások mentése](#14-eeprom--beállítások-mentése)
15. [Dual-core működés](#15-dual-core-működés)
16. [Tipikus használati folyamat](#16-tipikus-használati-folyamat)
...
- **BLE szerverként is működik egyidejűleg**: a fogadott szenzor adatokat változtatás nélkül továbbítja a csatlakozott BLE klienseknek (pl. Zwift app) – így **transzparens BLE híd**ként viselkedik,
...
## 8. BLE adattovábbítás – transzparens híd

Az ESP32-C3 **egyidejűleg BLE kliens és BLE szerver** is. Ez azt jelenti, hogy a fitnesz szenzortól érkező nyers BLE adatcsomagokat **azonnal, változtatás nélkül továbbítja** a saját BLE szervere felé – így bármely BLE kliens (pl. Zwift app, edzésnaplózó alkalmazás) csatlakozhat az ESP32-hez, és úgy látja az adatokat, mintha közvetlenül a szenzorhoz csatlakozna.

### Működési elv

```
Fitnesz szenzor  ──BLE──►  ESP32-C3 (BLE kliens)
                               │
                               │  notifyCallback()
                               │  pCharacteristic->setValue(pData, length)
                               │  pCharacteristic->notify()
                               ▼
                           ESP32-C3 (BLE szerver)  ──BLE──►  Zwift / más kliens
```

### Részletek

- **Kód helye:** `notifyCallback()` függvény, 782–783. sor
- **Rate limiting:** legfeljebb 10 ms-onként küldi tovább az adatot (nem terheli túl a BLE buszt)
- **Feltétel:** csak akkor továbbít, ha van csatlakozva kliens (`deviceConnected == true`)
- **Protokoll-átlátszóság:** az ugyanazon BLE Service UUID és Characteristic UUID alatt hirdetett szerver miatt a kliensek nem érzékelik a közvetítést
- Ha a kliens lecsatlakozik, az ESP32 automatikusan újra elkezd hirdetni (`startAdvertising()`), amint 500 ms eltelt

### Előny a gyakorlatban

Egyetlen szenzor egyszerre szolgálhat ki a ventilátor vezérlőt **és** a Zwift appot, anélkül hogy a szenzor natívan támogatná a több párhuzamos kapcsolatot.
...
## 9. WiFi funkciók és webes felület
...
## 16. Tipikus használati folyamat
